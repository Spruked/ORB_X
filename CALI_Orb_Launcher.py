...
class OrbDesktopWindow:
    ...
    def __init__(self):
        ...
        try:
            ...
            center_x = (screen_width - 256) // 2  # 256 is window width
            center_y = (screen_height - 256) // 2  # 256 is window height
    ...
    def start_orb(self):
        ...
        # Cursor following disabled for manual positioning
        # self.root.after(3000, self.start_cursor_following)  # Start following after 3 seconds
        print("[ORB] Cursor following disabled - manual positioning only")
    ...
    def on_close(self):
        ...
        print("[ORB-OVERLAY] ORB window closing, cleaning up...")
        remove_orb_lock()
        self.root.destroy()

def check_existing_orb() -> bool:
    """
    Check if an ORB instance is already running using a lock file.
    Returns True if an existing instance is found, False otherwise.
    """
    try:
        lock_file = Path(__file__).parent / ".orb_lock"
        if lock_file.exists():
            print("[LAUNCHER] ORB lock file exists, checking if process is still running...")
            try:
                ...
                if psutil.pid_exists(pid):
                    ...
                    if proc.is_running() and 'python' in proc.name().lower():
                        print(f"[LAUNCHER] ORB is already running (PID: {pid})")
                        return True
                    else:
                        print(f"[LAUNCHER] Process {pid} not running or not Python, removing stale lock file...")
                        lock_file.unlink()
                else:
                    print(f"[LAUNCHER] PID {pid} does not exist, removing stale lock file...")
                    lock_file.unlink()
            except (ValueError, psutil.NoSuchProcess, psutil.AccessDenied, FileNotFoundError):
                print("[LAUNCHER] Error checking lock file PID, removing...")
                try:
                    lock_file.unlink()
                except:
                    pass
            except Exception as e:
                print(f"[LAUNCHER] Error checking lock file: {e}")
                return False
        else:
            print("[LAUNCHER] No lock file found")

        return False

    except Exception as e:
        print(f"[LAUNCHER] Error in check_existing_orb: {e}")
        return False

def create_orb_lock() -> bool:
    """
    Create a lock file with the current process ID.
    Returns True if successful, False if lock already exists.
    """
    try:
        lock_file = Path(__file__).parent / ".orb_lock"
        print(f"[LAUNCHER] Attempting to create lock file: {lock_file}")

        # Check if file already exists
        if lock_file.exists():
            print("[LAUNCHER] Lock file already exists, checking if it's valid...")
            if check_existing_orb():
                return False  # Valid existing instance
            else:
                # Remove stale lock file
                try:
                    lock_file.unlink()
                    print("[LAUNCHER] Removed stale lock file")
                except Exception as e:
                    print(f"[LAUNCHER] Error removing stale lock file: {e}")
                    return False
        ...
        try:
            ...
            print(f"[LAUNCHER] Created ORB lock file with PID {pid}")

            return True

        except Exception as e:
            print(f"[LAUNCHER] Error writing lock file: {e}")
            return False

    except Exception as e:
        print(f"[LAUNCHER] Error creating lock file: {e}")
        return False

def remove_orb_lock() -> None:
    """
    ...
    try:
        ...
        if lock_file.exists():
            lock_file.unlink()
            print("[LAUNCHER] Removed ORB lock file")
    except Exception as e:
        print(f"[LAUNCHER] Error removing lock file: {e}")
...
def launch_orb(mode: str = "desktop"):
    ...
    """
    if mode == "desktop":
        # Check for existing ORB instances before launching
        if check_existing_orb():
            print("[LAUNCHER] ORB is already running. Exiting to prevent multiple instances.")
            return

        # Create lock file
        if not create_orb_lock():
            print("[LAUNCHER] Failed to create lock file, ORB may already be running.")
            return
